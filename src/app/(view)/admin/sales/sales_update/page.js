...
const SaleUpdate = ({ id }) => {
    ...
    const { data: saleProductSingle = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Admin/sales/sales_list/${id}`);
...
console.log(allData)
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/Admin/sales/sale_edit/${id}`, {
            method: 'POST',
            headers: {
    ...
    const unit_delete = id => {
        ...
        if (proceed) {
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/Admin/loan/sale_product_delete/${id}`, {
                method: "POST",
